...
Play Mode has no AI narration of its own; round outcomes are deterministic, plain-language strings
generated by `resolution.py`, not Claude. See [`enhancements.md`](enhancements.md) for that and
other deliberately deferred ideas (multi-character parties, status effects, a fifth "Use Item"
action, and more).
